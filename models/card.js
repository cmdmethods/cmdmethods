...
function sortByStrategy(a, b) {
    if (
        config.categories.indexOf(a.strategy) >
        config.categories.indexOf(b.strategy)
    ) {
        return 1;
    }
    if (
...
exports.findById = (id) => cardlist.find((el) => el.id === id);

exports.findPrevious = (id) => {
    const cards = cardlist.filter(
        (el) => el.status === 'active' && el.type === 'card'
    );
    cards.sort(sortbyStrategyTypeId);
...
exports.findByStrategy = (strategy) => {
    const cards = [...cardlist].filter(
        (el) => el.status === 'active' && el.strategy === strategy
    );
...
exports.filterBySearchString = (cards, searchString) =>
    [...cards].filter((el) => {
        ...
        if (el.why.toLowerCase().includes(searchString.toLowerCase())) {
            return 1;
        }
        if (el.how.toLowerCase().includes(searchString.toLowerCase())) {
            return 1;
        }
        if (el.in_practice.toLowerCase().includes(searchString.toLowerCase())) {
            return 1;
        }
        for (let i = 0; i < el.ingredients.length; i += 1) {
            if (
                el.ingredients[i]
                    .toLowerCase()
    ...
    [...cards].filter((card) => {
        if (card.sliders) {
            if (dimension === 'inspiration') {
                if (card.sliders.inspiration_data < 50) {
                    return 1;
                }
            }
            if (dimension === 'data') {
            ...
            if (dimension === 'expertise') {
                if (card.sliders.expertise_fit < 50) {
                    return 1;
                }
            }
            if (dimension === 'fit') {
            ...
            if (dimension === 'overview') {
                if (card.sliders.overview_certainty < 50) {
                    return 1;
                }
            }
...
exports.filterByStrategy = (cards, strategy) =>
    [...cards].filter((card) => card.strategy === strategy);
